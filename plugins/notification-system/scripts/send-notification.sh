...
main() {
    ...
    local last_response_time=$(read_state_file "$session_id" "last_response_time")
    ...
    # 空闲时长从 Claude 响应完成时算起（更准确）
    # 若 task-complete.sh 尚未记录（旧会话兼容），退回到 last_input_time
    local idle_base_time="${last_response_time:-}"
    if [[ -z "$idle_base_time" ]] || [[ "$idle_base_time" == "0" ]]; then
        idle_base_time="$last_input_time"
    fi
    local idle_duration=$((current_time - idle_base_time))
